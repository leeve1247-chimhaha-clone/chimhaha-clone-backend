...
public interface CommentRepository extends JpaRepository<Comment, Long> {
    ...
    @Query(value =  "with cte as (select id, " +
            ...
            "     cte2 as (select *, ceiling(accumulated_total / 5) as page from cte) " +
            "select min(id) as start_id, max(id) as end_id " +
            "from cte2\n " +
            "group by page;"
            , nativeQuery = true
    )
    List<CommentPage> getCommentPages(@Param("postId") Long postId);

    @Query("select sum(coalesce(c.replies_count, 0))+count(*) from Comment c where c.parent is null and c.post = :post")
    Integer countCommentsByPost(Post post);

    @Query(value = "with RECURSIVE cte as (select c.* from comments c where (c.parent_id IS NULL and c.post_id = :postId and :startId <= c.id <= :endId) union all select t.* from comments t inner join cte on t.parent_id = cte.id) select * from cte;" , nativeQuery = true)
