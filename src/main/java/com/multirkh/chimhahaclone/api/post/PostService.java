...
public class PostService {
    ...
    private void validatePostForm(PostReceived request) {
        ...
        if (request.getPostCategoryKey() == null) {
            throw new IllegalArgumentException("post category is null");
        }
    ...
    public PostDetailDto findPost(Long postNum) {
        Post post = postRepository.findByIdPostStatusNotDeleted(postNum)
                .orElseThrow(() -> new IllegalArgumentException("Post not Found"));
        if (post.getStatus() == PostStatus.DELETED) {
            throw new FindDeletedPostException();
        }
